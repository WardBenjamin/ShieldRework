...
public class ShieldClientOverride {
    ...
    private static EnumShieldBehavior configBehavior;
    ...
    public static void preInitClient(FMLPreInitializationEvent event) {
        ...
        OverrideBehavior(shield);
    }

    public static void OverrideBehavior(Item shield)
    {
        // Read from the configuration file if the configured behavior has not been set.
        if(configBehavior == null)
        {
            // Get the config option for shield behavior
            Configuration config = ShieldRework.config;
            config.load();
            int behaviorSetting = config.get(Configuration.CATEGORY_GENERAL, "shieldBehavior", 0).getInt();
            configBehavior = EnumShieldBehavior.fromInt(behaviorSetting);
            if(configBehavior == null)
                configBehavior = EnumShieldBehavior.DEFAULT;
            config.save();
        ...
        if (configBehavior == EnumShieldBehavior.LOWER_ALWAYS) {
        ...
        else if(configBehavior == EnumShieldBehavior.LOWER_WHEN_HOLDING_BLOCK) {
            shield.addPropertyOverride(new ResourceLocation("shieldrework", "blockheld"), new IItemPropertyGetter() {
                @Override
                public float apply(ItemStack stack, World worldIn, EntityLivingBase entityIn) {
        ...
        else if(configBehavior == EnumShieldBehavior.HIDE_COMPLETELY) {
